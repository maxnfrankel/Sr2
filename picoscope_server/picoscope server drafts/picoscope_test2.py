# the goal is to trigger the picoscope with an external trigger, record a time trace, and save it with the proper filename

# this code is based off of the examples shown here: https://github.com/picotech/picosdk-python-wrappers/tree/master/ps5000aExamples

# should trigger directly from conductor, if possible. Either that, or set up a timestamp with ntp. make sure it's synchronized with other computers
...
# ADC = analog-digital-converter count value
...
"""
# save the data from channel A
packed = {}
packed["time_ns"] = time
packed["ChA_mV"] = adc2mVChAMax

packedpath = pathlib.Path(__file__).parent.resolve()
